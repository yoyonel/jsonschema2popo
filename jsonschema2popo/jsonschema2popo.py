...
class JsonSchema2Popo:
    ...
    def __init__(self):
        self.jinja = Environment(loader=FileSystemLoader(searchpath=SCRIPT_DIR),
                                 trim_blocks=True)
    ...
    def definition_parser(self, _obj_name, _obj):
        ...
        if 'properties' in _obj:
            for _prop_name, _prop in _obj['properties'].items():
                ...
                if _type['type'] == list and 'items' in _prop and isinstance(
                        _prop['items'], list):
                    _format = _prop['items'][0]['format']
                else:
                    _format = _prop.get('format', None)
        ...
        if 'type' in t:
            if t['type'] == 'array' and 'items' in t:
                ...
                if isinstance(t['items'], list):
                    ...
                    elif '$ref' in t['items'] or 'oneOf' in t['items'] and len(
                            t['items']['oneOf']) == 1:
                        if '$ref' in t['items']:
                            ref = t['items']['$ref']
                        else:
    ...
    def write_file(self, filename):
        template = self.jinja.get_template(self.CLASS_TEMPLATE_FNAME)
        template = template.stream(models=self.definitions)
